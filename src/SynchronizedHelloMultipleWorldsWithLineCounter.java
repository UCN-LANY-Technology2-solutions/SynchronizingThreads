...
public class SynchronizedHelloMultipleWorldsWithLineCounter extends Thread {
	...
	// The line counter is also static, so it can be shared across instances of this
	// class
	private static int lineCounter = 0;

	public SynchronizedHelloMultipleWorldsWithLineCounter(int world) {
		this.world = world;
	}
		...
		synchronized (mutex) {
			for (int i = 0; i < 20; i++) {
				if (++lineCounter < 128) {
					...
					System.out.println(lineCounter + " lines printed");
